...
class Prius_model:
    def __init__(self):
        # paramsmeters of car
        self.Wheel_R = 0.287
        self.mass = 1449  # Curb weight 整备重量
        self.C_roll = 0.013  # Roll resistance coefficient 滚动阻力系数
        self.density_air = 1.2  # Air density 空气密度
        self.area_frontal = 2.23  # Frontal area 额叶面积
        self.G = 9.81  # Gravitational acceleration 重力加速度
        self.C_d = 0.26  # Air resistance coefficient 空气阻力系数
        self.T_factor = 1  # The factor of F_roll F_roll 的因子

        # paramsmeters of transmission system
        self.Pgs_R = 78  # Number of teeth on the ring 环上的齿数
        self.Pgs_S = 30  # Number of teeth on sun wheel 太阳轮的齿数
        self.Pgs_K = 3.93  # Speed ratio from ring gear to wheel 齿圈与车轮的速比

        # The optimal curve of engine 发动机最佳曲线
        ...
        Mot_trq_max_list = np.concatenate(
            (np.fliplr(Mot_trq_max_quarter[:, 1:]), Mot_trq_max_quarter), axis=1
        )
        # motor minimum torque
        Mot_trq_min_list = -Mot_trq_max_list
        self.Mot_trq_min_func = interp1d(
            Mot_spd_list, Mot_trq_min_list, kind="linear", fill_value="extrapolate"
        )
        self.Mot_trq_max_func = interp1d(
            Mot_spd_list, Mot_trq_max_list, kind="linear", fill_value="extrapolate"
        )
        ...
        self.Gen_trq_min_func = interp1d(
            Gen_spd_list, Gen_trq_min_list, kind="linear", fill_value="extrapolate"
        )
        self.Gen_trq_max_func = interp1d(
            Gen_spd_list, Gen_trq_max_list, kind="linear", fill_value="extrapolate"
        )

        # Battery
        # published capacity of one battery cell 电池单元的额定容量
        Batt_Q_cell = 6.5
        ...
        self.Batt_vol_func = interp1d(
            SOC_list, Batt_vol_list, kind="linear", fill_value="extrapolate"
        )
        self.Batt_rint_dis_list_func = interp1d(
            SOC_list, Batt_rint_dis_list, kind="linear", fill_value="extrapolate"
        )
        self.Batt_rint_chg_list_func = interp1d(
            SOC_list, Batt_rint_chg_list, kind="linear", fill_value="extrapolate"
        )
    ...
    def run(self, car_spd, car_a, Eng_pwr_opt, SOC):
        ...
        # Wheel torque (Nm)
        F_roll = (
            self.mass * self.G * self.C_roll * (self.T_factor if car_spd > 0 else 0)
        )
        ...
        Mot_trq = (
            (Mot_trq < 0)
            * (Mot_trq < self.Mot_trq_min_func(Mot_spd))
            * self.Mot_trq_min_func(Mot_spd)
            + (Mot_trq < 0) * (Mot_trq > self.Mot_trq_min_func(Mot_spd)) * Mot_trq
            + (Mot_trq >= 0)
            * (Mot_trq > self.Mot_trq_max_func(Mot_spd))
            * self.Mot_trq_max_func(Mot_spd)
            + (Mot_trq >= 0) * (Mot_trq < self.Mot_trq_max_func(Mot_spd)) * Mot_trq
        )

        Mot_trq = np.array(Mot_trq).flatten()
        Mot_eta = (Mot_spd == 0) + (Mot_spd != 0) * self.Mot_eta_map_func(
            Mot_trq, Mot_spd * np.ones(1)
        )  # need to edit
        inf_mot = (
            (np.isnan(Mot_eta))
            + (Mot_trq < 0) * (Mot_trq < self.Mot_trq_min_func(Mot_spd))
            + (Mot_trq >= 0) * (Mot_trq > self.Mot_trq_max_func(Mot_spd))
        )
        ...
        Gen_spd = (
            Eng_spd * (self.Pgs_R + self.Pgs_S) - Mot_spd * self.Pgs_R
        ) / self.Pgs_S
        Gen_trq = -F_pgs * self.Pgs_S
        Gen_eta = (Gen_spd == 0) + (Gen_spd != 0) * self.Gen_eta_map_func(
            Gen_trq, Gen_spd
        )
        inf_gen = (
            (np.isnan(Gen_eta))
            + (Gen_trq < 0) * (Gen_trq < self.Gen_trq_min_func(Gen_spd))
            + (Gen_trq >= 0) * (Gen_trq > self.Gen_trq_max_func(Gen_spd))
        )
        ...
        Batt_rint = (Batt_pwr > 0) * self.Batt_rint_dis_list_func(SOC) + (
            Batt_pwr <= 0
        ) * self.Batt_rint_chg_list_func(SOC)
        ...
        #        print('battery power is out of bound')
            ...
            Batt_I = (
                Batt_eta
                * (Batt_vol - np.sqrt(Batt_vol**2 - 4 * Batt_rint * Batt_pwr))
                / 0.8
            )
