...
# Added import for traci to interact with SUMO simulator
import traci
# Added code to start SUMO simulation
sumoCmd = ['sumo', '-c', 'path_to_your_sumo_config_file.sumocfg']
traci.start(sumoCmd)
...
# Replaced 'car_spd' with traci's getSpeed function for the vehicle with ID 'agent'
traci.vehicle.getSpeed('agent')_one = data["speed_vector"]
        total_milage = np.sum(traci.vehicle.getSpeed('agent')_one) / 1000
        ...
        Mot_eta_list = []
        Gen_eta_list = []
        traci.vehicle.getSpeed('agent') = traci.vehicle.getSpeed('agent')_one[:, 0]
        
# Replaced 'car_a' with traci's getAcceleration function for the vehicle with ID 'agent'
traci.vehicle.getAcceleration('agent') = traci.vehicle.getSpeed('agent')_one[:, 0] - 0
        s = np.zeros(s_dim)
        s[0] = traci.vehicle.getSpeed('agent') / 24.1683
        s[1] = (traci.vehicle.getAcceleration('agent') - (-1.6114)) / (1.3034 - (-1.6114))
        s[2] = SOC
        ...
        for j in range(traci.vehicle.getSpeed('agent')_one.shape[1] - 1):
            #print(str(i) + " ---> " + str(j) + "/", traci.vehicle.getSpeed('agent')_one.shape[1])
            ...
            out, cost, I = Prius.run(traci.vehicle.getSpeed('agent'), traci.vehicle.getAcceleration('agent'), Eng_pwr_opt, SOC)
            ...
            traci.vehicle.getSpeed('agent') = traci.vehicle.getSpeed('agent')_one[:, j + 1]
            traci.vehicle.getAcceleration('agent') = traci.vehicle.getSpeed('agent')_one[:, j + 1] - traci.vehicle.getSpeed('agent')_one[:, j]
            s_ = np.zeros(s_dim)
            s_[0] = traci.vehicle.getSpeed('agent') / 24.1683
            s_[1] = (traci.vehicle.getAcceleration('agent') - (-1.6114)) / (1.3034 - (-1.6114))
            s_[2] = SOC_new
            ...
            if j == (traci.vehicle.getSpeed('agent')_one.shape[1] - 2):
...
def test_ddpg():
    ...
    for j in range(traci.vehicle.getSpeed('agent')_one.shape[1] - 1):
        #print(str(j) + "/", traci.vehicle.getSpeed('agent')_one.shape[1])
        action, action_no_noise = ddpg.choose_action(
            s, j, param_noise_scale, False, action_noise_type
        )
        adaptive_policy_distance = np.sqrt(
