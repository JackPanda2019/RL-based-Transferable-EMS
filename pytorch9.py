...
def run_ddpg():
    ...
    for i in range(MAX_EPISODES):
        ...
        while traci.simulation.getTime() < end_time:
            ...
            inf_batt_one_count+=int(out["inf_batt_one"])
            inf_batt_count+=int(out["inf_batt"])
            I_count+=int(I)
        ...
        writer.add_scalar("inf_batt_one_count", inf_batt_one_count, i)
        print("Episode:",i," cost_Engine: %.3f" % cost_Engine," reward: %.3f" % (ep_reward_all)," SOC-final: %.3f" % SOC,
              "inf_batt_one_count:",inf_batt_one_count, "inf_batt_count:",inf_batt_count, "I_count:",I_count, "SOC_penalty_count:",SOC_penalty_count)
...
def test_ddpg():
    ...
    with pd.ExcelWriter(f"output{time_string_result}.xlsx") as data_writer:
    ...
    cost_Engine += ((SOC < SOC_origin)*(SOC_origin - SOC)*(201.6 * 6.5)* 3600/ (42600000)/ 0.72)  
    print("  reward:", ep_reward_all, "  Engine_cost:", cost_Engine, "  SOC:", SOC, "  inf_batt_one_count:", inf_batt_one_count)
    results = results.append({
        'ep_reward_all': ep_reward_all, 
        'cost_Engine': cost_Engine, 
        'SOC': SOC, 
        'inf_batt_one_count': inf_batt_one_count
    }, ignore_index=True)
    #data = {'ep_reward_all': [ep_reward_all],'cost_Engine': [cost_Engine],'SOC': [SOC],'inf_batt_one_count': [inf_batt_one_count]}
    #df = pd.DataFrame(data)
    #df.to_excel(f"output{time_string2}_result.xlsx", sheet_name="result", index=False)
    traci.close()
...
run_ddpg()
print("train %s is done, test:"%time_string)
results = pd.DataFrame(columns=['ep_reward_all', 'cost_Engine', 'SOC', 'inf_batt_one_count'])
for i in range(1,11):
    time_string_result = time_string+"_"+str(i)
    print("time_string_result:",time_string_result)
    test_ddpg()
results.to_excel(f"output{time_string}_result_all.xlsx", sheet_name="result", index=False)
