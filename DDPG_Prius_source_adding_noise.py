...
MAX_EPISODES = 100  # The max episode
...
class DDPG(object):
    def __init__(
        ...
        self.soft_replace = [
            ...
            for ta, ea, tc, ec in zip(
                self.at_params, self.ae_params, self.ct_params, self.ce_params
            )
        ]
    ...
    def OrnsteinUhlenbeckActionNoise(self, mu, sigma, theta=0.15, dt=1e-2):
        ou_noise = (
            ...
            + sigma * np.sqrt(dt) * np.random.normal(mu, sigma)
        )
        self.ou_noise_prev = ou_noise
        return ou_noise

    def choose_action(
        self, s, loop, param_noise_scale, param_noise=True, action_noise_type=None
    ):
            ...
            for ea in self.ae_params:
                updates.append(
                    tf.assign(
                        ...
                        ea
                        + tf.random_normal(
                            tf.shape(ea), mean=0.0, stddev=param_noise_scale
                        ),
                    )
                )
    ...
    def learn(self):
        ...
        abs_td_error = self.sess.run(
            self.td_error_up, {self.S: bs, self.a: ba, self.R: br, self.S_: bs_}
        )
    ...
    def _build_a(self, s, scope, trainable):
        ...
        with tf.variable_scope(scope):
            net1 = tf.layers.dense(
                s, 200, activation=tf.nn.relu, name="l1", trainable=trainable
            )
            net2 = tf.layers.dense(
                net1, 100, activation=tf.nn.relu, name="l2", trainable=trainable
            )
            net3 = tf.layers.dense(
                net2, 50, activation=tf.nn.relu, name="l3", trainable=trainable
            )
            a = tf.layers.dense(
                net3,
                self.a_dim,
    ...
    def savemodel(self):
        self.saver.save(
            self.sess, "Checkpoints/source/save_net.ckpt", global_step=step_episode
        )
